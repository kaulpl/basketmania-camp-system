...
class BCS_CRM {
    ...
    private static function camp_form_accordion(object $r): string {
        $rows=[
            ['Opiekun',$r->parent_first_name.' '.$r->parent_last_name],['Imiona i nazwiska rodziców',$r->parents_names??''],['E-mail',$r->parent_email],['Telefon I',$r->parent_phone],['Telefon II',$r->parent_phone_alt??''],['Kod pocztowy',$r->parent_postal_code],['Miejscowość',$r->parent_city],['Ulica',$r->parent_street],['Nr domu / lokalu',$r->parent_house_number],
            ['Uczestnik',$r->child_first_name.' '.$r->child_last_name],['Adres uczestnika (jeżeli inny)',$r->child_address??''],['Data urodzenia',$r->child_birth_date],['PESEL',$r->child_pesel],['Wzrost',$r->child_height?((int)$r->child_height.' cm'):''],['Waga',!empty($r->child_weight)?$r->child_weight.' kg':''],['Rozmiar stroju',$r->shirt_size],['Klub',$r->child_club],
            ['Specjalne potrzeby edukacyjne',$r->special_educational_needs??''],['Uwagi zdrowotne',$r->medical_notes],['Dieta i żywienie',$r->dietary_notes],['Szczepienie przeciw tężcowi – rok',$r->vaccination_tetanus??''],['Szczepienie przeciw błonicy – rok',$r->vaccination_diphtheria??''],['Inne szczepienia',$r->vaccination_other??''],['Kontakt podczas pobytu',$r->stay_contact],['Osoby upoważnione do odbioru',$r->authorized_pickup],['Dodatkowe informacje dla organizatora',$r->camp_notes],
            ['Faktura',!empty($r->invoice_requested)?'tak':'nie'],['Nabywca faktury',$r->invoice_buyer_name??''],['Ulica do faktury',$r->invoice_street??''],['Kod pocztowy do faktury',$r->invoice_postal_code??''],['Miejscowość do faktury',$r->invoice_city??''],['NIP nabywcy',$r->invoice_nip??''],['Dodatkowe dane na fakturze',$r->invoice_notes??''],['Turnus',$r->camp_name],['Termin',$r->start_date.' – '.$r->end_date],['Miejsce',$r->location]
        ];
        ...
        foreach($rows as [$label,$value]){$display=trim((string)$value)!==''?(string)$value:'—';$wide=in_array($label,['Specjalne potrzeby edukacyjne','Uwagi zdrowotne','Dieta i żywienie','Inne szczepienia','Dodatkowe informacje dla organizatora','Dodatkowe dane na fakturze'],true)?' bcs-detail-wide':'';$html.='<div class="bcs-detail-item'.$wide.'"><span>'.esc_html($label).'</span><strong>'.nl2br(esc_html($display)).'</strong></div>';}
