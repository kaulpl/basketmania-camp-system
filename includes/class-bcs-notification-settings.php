...
final class BCS_Notification_Settings {
    ...
    private static function settings_html(array $saved): string {
        ...
        $html='<div class="bcs-panel-head"><div><h2>Powiadomienia workflow</h2><p>Każde zdarzenie ma niezależne ustawienie kanału oraz edytowalny szablon. Brak szablonu jest oznaczany jako błąd konfiguracji.</p></div><a class="button" href="'.esc_url(admin_url('admin.php?page=bcs-templates')).'">Otwórz wszystkie szablony</a></div>';
        ...
        $html.='<table class="widefat striped"><thead><tr><th>Etap procesu</th><th>Kiedy jest wysyłane</th><th>Szablon</th><th>Kanał</th></tr></thead><tbody>';
        $options=['off'=>'Nie wysyłaj','email'=>'E-mail','sms'=>'SMS','both'=>'E-mail i SMS'];
        foreach(self::events() as $key=>$event){
            $current=(string)($saved[$key]??$event['default']);$exists=!empty($email_templates[$key]);
            ...
            $html.='<td>'.($exists?'<span style="color:#16803c;font-weight:700">✓ Dostępny</span>':'<span style="color:#b32d2e;font-weight:700">✕ Brak</span>').'<br><a href="'.esc_url($edit).'">Edytuj treść e-mail i SMS</a></td><td><select name="channels['.esc_attr($key).']">';
            foreach($options as $value=>$label)$html.='<option value="'.esc_attr($value).'" '.selected($current,$value,false).'>'.esc_html($label).'</option>';
            $html.='</select></td></tr>';
        }
        $html.='</tbody></table><p class="description">Zasada systemowa: każde nowe automatyczne powiadomienie musi mieć wpis w tej tabeli oraz edytowalny szablon w module Szablony.</p><p><button class="button button-primary button-hero">Zapisz ustawienia powiadomień</button></p></form>';
        return $html;
    }
}
