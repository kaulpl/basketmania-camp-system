...
class BCS_Frontend {
    ...
    public static function handle_signup(): void {
        ...
        $now=BCS_Utils::now();$token=BCS_Utils::random_token();$user_agent=sanitize_text_field(wp_unslash($_SERVER['HTTP_USER_AGENT']??''));$device_type=self::detect_device_type($user_agent);$camp_year=(int)substr((string)$camp->start_date,0,4);if($camp_year<2000)$camp_year=(int)current_time('Y');
        $ok=$wpdb->insert(BCS_DB::table('registrations'),['camp_id'=>$camp_id,'public_token'=>$token,'status'=>'admin_confirmed','form_status'=>'incomplete','source'=>'website_form','device_type'=>$device_type,'device_user_agent'=>$user_agent,'parent_first_name'=>$parent_first,'parent_last_name'=>$parent_last,'parent_email'=>sanitize_email(wp_unslash($_POST['parent_email']??'')),'parent_phone'=>sanitize_text_field(wp_unslash($_POST['parent_phone']??'')),'child_first_name'=>sanitize_text_field(wp_unslash($_POST['child_first_name']??'')),'child_last_name'=>sanitize_text_field(wp_unslash($_POST['child_last_name']??'')),'child_birth_date'=>sanitize_text_field(wp_unslash($_POST['child_birth_date']??'')),'child_height'=>absint($_POST['child_height']??0),'shirt_size'=>sanitize_text_field(wp_unslash($_POST['shirt_size']??'')),'total_amount'=>(float)$camp->price,'paid_amount'=>0,'admin_confirmed_at'=>$now,'agreement_available_from'=>sprintf('%04d-01-01',$camp_year),'created_at'=>$now,'updated_at'=>$now]);
        ...
        wp_safe_redirect(add_query_arg('edit','camp',self::portal_url($token)));exit;
    }
    public static function complete_registration(): void {
        ...
        $data=[
            ...
            'parent_phone_alt'=>sanitize_text_field(wp_unslash($_POST['parent_phone_alt']??($r->parent_phone_alt??''))),
            'parents_names'=>sanitize_textarea_field(wp_unslash($_POST['parents_names']??($r->parents_names??''))),
            ...
            'child_weight'=>max(0,(float)($_POST['child_weight']??($r->child_weight??0))),
            ...
            'child_address'=>sanitize_textarea_field(wp_unslash($_POST['child_address']??($r->child_address??''))),
            ...
            'special_educational_needs'=>$fallback(sanitize_textarea_field(wp_unslash($_POST['special_educational_needs']??($r->special_educational_needs??'')))),
            'vaccination_tetanus'=>sanitize_text_field(wp_unslash($_POST['vaccination_tetanus']??($r->vaccination_tetanus??''))),
            'vaccination_diphtheria'=>sanitize_text_field(wp_unslash($_POST['vaccination_diphtheria']??($r->vaccination_diphtheria??''))),
            'vaccination_other'=>sanitize_textarea_field(wp_unslash($_POST['vaccination_other']??($r->vaccination_other??''))),
            ...
            'invoice_requested'=>isset($_POST['invoice_requested'])?1:0,
            'invoice_buyer_name'=>sanitize_text_field(wp_unslash($_POST['invoice_buyer_name']??($r->invoice_buyer_name??''))),
            'invoice_street'=>sanitize_text_field(wp_unslash($_POST['invoice_street']??($r->invoice_street??''))),
            'invoice_postal_code'=>sanitize_text_field(wp_unslash($_POST['invoice_postal_code']??($r->invoice_postal_code??''))),
            'invoice_city'=>sanitize_text_field(wp_unslash($_POST['invoice_city']??($r->invoice_city??''))),
            'invoice_nip'=>sanitize_text_field(wp_unslash($_POST['invoice_nip']??($r->invoice_nip??''))),
            'invoice_notes'=>sanitize_textarea_field(wp_unslash($_POST['invoice_notes']??($r->invoice_notes??''))),
        ...
        if (is_wp_error($parents)) wp_die(esc_html($parents->get_error_message()), '', ['response'=>422]);
    ...
    private static function portal_status_message(object $r): array {
        ...
        if($paid) return ['Płatność zaksięgowana','Dziękujemy. Oczekujemy na podpisy karty kwalifikacyjnej przez wymaganych rodziców, a następnie organizatora. Dopiero po tych podpisach będzie można wygenerować fakturę.'];
        ...
        if(!empty($r->form_verified_at)) return ['Formularz zaakceptowany','Organizator zaakceptował formularz. Umowa oczekuje na wysłanie do podpisu przez organizatora.'];
    ...
    public static function render_portal(?array $qualification_context=null): string {
        ...
        $panel_url=$qualification_context?BCS_Qualification::portal_url((int)$r->id,$qualification_context['role'],$qualification_context['token']):self::portal_url($token);
        $qualification=BCS_Qualification::card((int)$r->id);
        $qualification_done=$qualification && BCS_Qualification::stage($qualification)==='card_signed';
        $qualification_label=$qualification?(BCS_Workflow::statuses()[BCS_Qualification::stage($qualification)]??'Karta kwalifikacyjna'):'Dostępna po pełnej wpłacie';
        $render_form=function()use($r,$token){
            ...
            <section class="bcs-form-section"><h3>Pierwszy rodzic / opiekun prawny</h3><div class="bcs-grid"><label>Imię opiekuna<input name="parent_first_name" value="<?php echo esc_attr((string)($r->parent_first_name??''));?>" required></label><label>Nazwisko opiekuna<input name="parent_last_name" value="<?php echo esc_attr((string)($r->parent_last_name??''));?>" required></label><label>E-mail<input type="email" name="parent_email" value="<?php echo esc_attr((string)($r->parent_email??''));?>" required></label><label>Telefon I<input name="parent_phone" value="<?php echo esc_attr((string)($r->parent_phone??''));?>" required></label><?php echo BCS_Qualification::parent_fields($r);?><label>Kod pocztowy<input name="parent_postal_code" value="<?php echo esc_attr((string)($r->parent_postal_code??''));?>" required></label><label>Miejscowość<input name="parent_city" value="<?php echo esc_attr((string)($r->parent_city??''));?>" required></label><label>Ulica<input name="parent_street" value="<?php echo esc_attr((string)($r->parent_street??''));?>" required></label><label>Nr domu / lokalu<input name="parent_house_number" value="<?php echo esc_attr((string)($r->parent_house_number??''));?>" required></label><input type="hidden" name="parent_address" value="<?php echo esc_attr((string)($r->parent_address??''));?>"></div></section>
            <section class="bcs-form-section"><h3>Uczestnik obozu</h3><div class="bcs-grid"><label>Imię uczestnika<input name="child_first_name" value="<?php echo esc_attr((string)($r->child_first_name??''));?>" required></label><label>Nazwisko uczestnika<input name="child_last_name" value="<?php echo esc_attr((string)($r->child_last_name??''));?>" required></label><label>Data urodzenia<input type="date" name="child_birth_date" value="<?php echo esc_attr((string)($r->child_birth_date??''));?>" required></label><label>PESEL<input name="child_pesel" inputmode="numeric" maxlength="11" pattern="[0-9]{11}" value="<?php echo esc_attr((string)($r->child_pesel??''));?>" required></label><label>Wzrost (cm)<input type="number" min="100" max="230" name="child_height" value="<?php echo esc_attr((string)($r->child_height??''));?>" required></label><label>Waga (kg)<input type="number" min="20" max="200" step="0.1" name="child_weight" value="<?php echo esc_attr((string)($r->child_weight??''));?>" required></label><label>Rozmiar stroju<input name="shirt_size" value="<?php echo esc_attr((string)($r->shirt_size??''));?>" required></label><label>Klub<input name="child_club" value="<?php echo esc_attr(($r->child_club??'')==='brak'?'':(string)($r->child_club??''));?>"></label><label class="bcs-span">Adres uczestnika, jeżeli inny niż adres opiekuna<textarea name="child_address"><?php echo esc_textarea((string)($r->child_address??''));?></textarea><small>Pozostaw puste, jeśli adres jest taki sam.</small></label><label class="bcs-span">Specjalne potrzeby edukacyjne<textarea name="special_educational_needs"><?php echo esc_textarea(($r->special_educational_needs??'')==='brak'?'':(string)($r->special_educational_needs??''));?></textarea><small>W tym potrzeby wynikające z niepełnosprawności lub niedostosowania społecznego. Wpisz „brak”, jeśli nie występują.</small></label><label class="bcs-span">Istotne informacje zdrowotne, rozwój psychofizyczny, alergie i leki<textarea name="medical_notes"><?php echo esc_textarea(($r->medical_notes??'')==='brak'?'':(string)($r->medical_notes??''));?></textarea><small>Uwzględnij choroby przewlekłe, urazy, dawkowanie leków, okulary/aparat oraz tolerancję jazdy samochodem. Wpisz „brak”, jeśli nie występują.</small></label><label class="bcs-span">Dieta, alergie i nietolerancje pokarmowe<textarea name="dietary_notes"><?php echo esc_textarea(($r->dietary_notes??'')==='brak'?'':(string)($r->dietary_notes??''));?></textarea><small>Wpisz „brak”, jeśli nie występują.</small></label><label>Szczepienie przeciw tężcowi – rok<input name="vaccination_tetanus" inputmode="numeric" maxlength="4" pattern="[0-9]{4}" value="<?php echo esc_attr((string)($r->vaccination_tetanus??''));?>"></label><label>Szczepienie przeciw błonicy – rok<input name="vaccination_diphtheria" inputmode="numeric" maxlength="4" pattern="[0-9]{4}" value="<?php echo esc_attr((string)($r->vaccination_diphtheria??''));?>"></label><label class="bcs-span">Inne szczepienia<textarea name="vaccination_other"><?php echo esc_textarea((string)($r->vaccination_other??''));?></textarea></label></div></section>
            ...
            <section class="bcs-form-section"><h3>Dane do faktury</h3><label class="bcs-check bcs-check-left"><input type="checkbox" name="invoice_requested" value="1" <?php checked(!empty($r->invoice_requested));?>> <span>Chcę otrzymać fakturę na poniższe dane</span></label><div class="bcs-grid"><label class="bcs-span">Imię i nazwisko / nazwa firmy<input name="invoice_buyer_name" value="<?php echo esc_attr((string)($r->invoice_buyer_name??''));?>"></label><label>Ulica i numer<input name="invoice_street" value="<?php echo esc_attr((string)($r->invoice_street??''));?>"></label><label>Kod pocztowy<input name="invoice_postal_code" value="<?php echo esc_attr((string)($r->invoice_postal_code??''));?>"></label><label>Miejscowość<input name="invoice_city" value="<?php echo esc_attr((string)($r->invoice_city??''));?>"></label><label>NIP firmy<input name="invoice_nip" value="<?php echo esc_attr((string)($r->invoice_nip??''));?>"></label><label class="bcs-span">Dodatkowe dane na fakturze<textarea name="invoice_notes"><?php echo esc_textarea((string)($r->invoice_notes??''));?></textarea></label></div><small>Jeżeli nie uzupełnisz danych nabywcy, faktura zostanie wystawiona na dane opiekuna.</small></section>
