...
class BCS_Feedback {
        ...
        <div class="bcs-feedback-modal" data-bcs-feedback-modal hidden>
            ...
            <section class="bcs-feedback-dialog" role="dialog" aria-modal="true" aria-labelledby="bcs-feedback-title">
                ...
                <div class="bcs-feedback-dialog-head">
                    <span class="dashicons dashicons-megaphone" aria-hidden="true"></span>
                    <div>
                        <h2 id="bcs-feedback-title">Nowe zgłoszenie</h2>
                        <p>Zgłaszasz uwagę w module: <strong><?php echo esc_html(self::current_module()); ?></strong></p>
                    </div>
                </div>
                <form data-bcs-feedback-form>
                    <label for="bcs-feedback-type">Rodzaj zgłoszenia</label>
                    <select id="bcs-feedback-type" name="type">
                        <option value="bug">Błąd</option>
                        <option value="feature">Nowa funkcjonalność</option>
                        <option value="improvement">Poprawka / usprawnienie</option>
                    </select>
                    ...
                    <div class="bcs-feedback-actions">
                        <button type="button" class="button" data-bcs-feedback-close>Anuluj</button>
    ...
    public static function handle_status_action(): void {
        if (!current_user_can(self::CAPABILITY)) return;
        if (!isset($_POST['bcs_feedback_status'])) return;
        ...
        if ($status === 'resolved') {
            ...
            $data['resolved_at'] = BCS_Utils::now();
            $formats = ['%s','%s','%d','%s'];
        }
        $wpdb->update(BCS_DB::table('feedback'), $data, ['id' => $id], $formats, ['%d']);
        wp_safe_redirect(add_query_arg(['page' => 'bcs-feedback', 'status_filter' => sanitize_key($_GET['status_filter'] ?? ''), 'updated' => 1], admin_url('admin.php')));
        exit;
    }
    ...
    public static function page(): void {
        ...
        $where = '';
        if (in_array($filter, self::STATUSES, true)) $where = $wpdb->prepare(' WHERE status=%s', $filter);
        $rows = $wpdb->get_results("SELECT * FROM " . BCS_DB::table('feedback') . $where . " ORDER BY created_at DESC, id DESC LIMIT 500");
        $counts = [];
        foreach (self::STATUSES as $status) {
            $counts[$status] = (int) $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM " . BCS_DB::table('feedback') . " WHERE status=%s", $status));
        }
        ...
        echo '<div class="wrap bcs-admin bcs-feedback-page">';
        echo '<div class="bcs-page-head"><div><h1>Feedback</h1><p>Zgłoszenia błędów, poprawek i nowych funkcjonalności od administratorów systemu.</p></div><span class="bcs-count">'.count($rows).' zgłoszeń</span></div>';
        ...
        echo '</nav>';
        echo '<div class="bcs-panel bcs-feedback-list">';
        if (!$rows) {
            ...
            foreach ($rows as $row) {
                echo '<tr>';
                echo '<td><strong>#'.(int)$row->id.'</strong></td>';
                ...
                echo '<td class="bcs-feedback-description">'.nl2br(esc_html($row->description)).'</td>';
                echo '<td><span class="bcs-feedback-status status-'.esc_attr($row->status).'">'.esc_html($status_labels[$row->status] ?? $row->status).'</span></td>';
                echo '<td><div class="bcs-feedback-row-actions">';
                foreach (['in_progress'=>'W trakcie','resolved'=>'Rozwiązano','cancelled'=>'Anulowano'] as $status => $label) {
                    if ($row->status === $status) continue;
                    echo '<form method="post">';
                    wp_nonce_field('bcs_feedback_status_' . (int)$row->id);
                    echo '<input type="hidden" name="feedback_id" value="'.(int)$row->id.'"><input type="hidden" name="status" value="'.esc_attr($status).'">';
                    echo '<button class="button bcs-feedback-action status-'.esc_attr($status).'" name="bcs_feedback_status" value="1">'.esc_html($label).'</button></form>';
                }
                echo '</div></td></tr>';
            }
