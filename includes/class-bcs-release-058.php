...
final class BCS_Release_058 {
    ...
    public static function init(): void {
        ...
        // Priorytet 5 jest celowy: przenosimy blok weryfikacji przed skryptem 0.42,
        // który zapamiętuje zawartość rozwijanej sekcji na potrzeby edycji i anulowania.
    ...
    private static function display_value(mixed $value, string $suffix = ''): string {
        $text = trim((string)$value);
        if ($text === '') return '—';
        return esc_html($text.$suffix);
    }

    private static function preview_sections(object $r): array {
        $invoice_requested = !empty($r->invoice_requested) ? 'tak' : 'nie';
        return [
    ...
    private static function preview_html(object $r): string {
        ...
        foreach (self::preview_sections($r) as $heading => $rows) {
            $html .= '<section class="bcs-form-review-section-058"><h3>'.esc_html($heading).'</h3><div class="bcs-form-review-grid-058">';
            foreach ($rows as $row) {
                $label = (string)($row[0] ?? '');
                $value = $row[1] ?? '';
                $wide = !empty($row[2]);
                $display = trim((string)$value) !== '' ? nl2br(esc_html((string)$value)) : '—';
                $html .= '<div class="bcs-form-review-item-058'.($wide ? ' is-wide' : '').'"><span>'.esc_html($label).'</span><strong>'.$display.'</strong></div>';
            }
            $html .= '</div></section>';
        }
        return $html.'</div>';
    ...
    public static function admin_footer(): void {
        if (!self::is_registrations_page() || !current_user_can('manage_options')) return;
        ...
        <style id="bcs-form-review-style-058">
            ...
            .bcs-form-verification-inline-058 h2{margin-top:0}
            .bcs-form-verification-inline-058 form{display:flex;justify-content:flex-end;margin-top:14px}
            ...
            #bcs-form-review-modal-058 .bcs-form-review-head-058{padding:22px 26px 16px;border-bottom:1px solid #e5e7eb}
            #bcs-form-review-modal-058 .bcs-form-review-head-058 h2{margin:0 40px 6px 0}
            #bcs-form-review-modal-058 .bcs-form-review-head-058 p{margin:0;color:#5b6472}
            #bcs-form-review-modal-058 .bcs-form-review-scroll-058{padding:20px 26px;overflow:auto;overscroll-behavior:contain}
            #bcs-form-review-modal-058 .bcs-form-review-footer-058{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:16px 26px;border-top:1px solid #e5e7eb;background:#f8fafc}
            #bcs-form-review-modal-058 .bcs-form-review-result-058{margin:0;line-height:1.45;color:#475569}
            #bcs-form-review-modal-058 .bcs-form-review-result-058.is-success{color:#166534;font-weight:700}
            #bcs-form-review-modal-058 .bcs-form-review-result-058.is-error{color:#b91c1c;font-weight:700}
            #bcs-form-review-modal-058 .bcs-form-review-actions-058{display:flex;gap:10px;flex:0 0 auto}
            .bcs-form-review-section-058{margin:0 0 20px}
            .bcs-form-review-section-058:last-child{margin-bottom:0}
            .bcs-form-review-section-058 h3{margin:0 0 10px;padding-bottom:8px;border-bottom:2px solid #f97316;color:#172033}
            .bcs-form-review-grid-058{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px 14px}
            .bcs-form-review-item-058{padding:10px 12px;border:1px solid #e2e8f0;border-radius:8px;background:#fff}
            .bcs-form-review-item-058.is-wide{grid-column:1/-1}
            .bcs-form-review-item-058 span{display:block;margin-bottom:4px;font-size:12px;color:#64748b}
            .bcs-form-review-item-058 strong{display:block;line-height:1.45;color:#172033;word-break:break-word}
            .bcs-form-review-loading-058{padding:36px;text-align:center;color:#64748b}
            @media(max-width:700px){
                .bcs-form-review-grid-058{grid-template-columns:1fr}
                .bcs-form-review-item-058.is-wide{grid-column:auto}
                #bcs-form-review-modal-058 .bcs-form-review-footer-058{align-items:stretch;flex-direction:column}
                #bcs-form-review-modal-058 .bcs-form-review-actions-058{justify-content:flex-end}
            }
        </style>
        <div id="bcs-form-review-modal-058" class="bcs-contact-modal" hidden>
            <div class="bcs-contact-modal__dialog" role="dialog" aria-modal="true" aria-labelledby="bcs-form-review-title-058">
                ...
                <div class="bcs-form-review-head-058">
                    <h2 id="bcs-form-review-title-058">Formularz obozowy</h2>
                    <p data-bcs-form-review-description-058>Sprawdź komplet danych przed potwierdzeniem poprawności formularza.</p>
                </div>
                <div class="bcs-form-review-scroll-058" data-bcs-form-review-content-058>
                    <div class="bcs-form-review-loading-058">Pobieranie formularza…</div>
                </div>
                <div class="bcs-form-review-footer-058">
                    <p class="bcs-form-review-result-058" data-bcs-form-review-result-058></p>
                    <div class="bcs-form-review-actions-058">
                        <button type="button" class="button" data-bcs-form-review-close-058>Zamknij</button>
                        <button type="button" class="button button-primary" data-bcs-form-review-confirm-058 disabled>Potwierdź poprawność formularza</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        (() => {
            ...
            const openShell = () => {
                ...
                title.textContent = 'Formularz obozowy';
                description.textContent = 'Sprawdź komplet danych przed potwierdzeniem poprawności formularza.';
                content.innerHTML = '<div class="bcs-form-review-loading-058">Pobieranie formularza…</div>';
                result.textContent = '';
                result.className = 'bcs-form-review-result-058';
                confirmButton.hidden = false;
                confirmButton.disabled = true;
                confirmButton.textContent = 'Potwierdź poprawność formularza';
            };
            ...
            const post = async (data) => {
                ...
                catch (error) { throw new Error('Serwer zwrócił nieprawidłową odpowiedź. Odśwież stronę i spróbuj ponownie.'); }
            ...
            const applyRowState = (row, data) => {
                ...
                } else if (!data.requires_action && marker) {
                    marker.remove();
                }
                row.classList.add('bcs-ajax-updated-02013');
                window.setTimeout(() => row.classList.remove('bcs-ajax-updated-02013'), 1600);
                enhanceListButtons(row);
            };

            const openReview = async (button) => {
                const form = button.closest('form');
                const row = button.closest('tr[data-id]');
                const registrationId = form?.querySelector('[name="registration_id"]')?.value || row?.dataset.id || '';
                const nonce = form?.querySelector('[name="_wpnonce"]')?.value || '';
                if (!registrationId || !nonce) {
                    popup('Brakuje danych zgłoszenia albo sesja wygasła.', false);
                    return;
                }
                openShell();
                current = {registrationId, nonce, row};
                try {
                    ...
                    title.textContent = data.title || 'Formularz obozowy';
                    description.textContent = data.message || 'Sprawdź wszystkie dane przed potwierdzeniem.';
                    content.innerHTML = data.html || '<p>Brak danych formularza.</p>';
                    confirmButton.disabled = !data.can_verify;
                    confirmButton.hidden = !data.can_verify;
                } catch (error) {
                    content.innerHTML = '<div class="notice notice-error inline"><p>'+String(error.message || 'Nie udało się pobrać formularza.')+'</p></div>';
                    result.textContent = error.message || 'Nie udało się pobrać formularza.';
                    result.className = 'bcs-form-review-result-058 is-error';
                }
            };
            ...
            const verifyCurrent = async () => {
                ...
                result.textContent = 'Trwa potwierdzanie formularza i przygotowanie draftu umowy.';
                result.className = 'bcs-form-review-result-058';
                try {
                    ...
                    applyRowState(current.row, data);
                    result.textContent = data.message || 'Formularz obozowy został zaakceptowany.';
                    result.className = 'bcs-form-review-result-058 is-success';
                    confirmButton.hidden = true;
                    popup(result.textContent, true);
                } catch (error) {
                    result.textContent = error.message || 'Nie udało się potwierdzić formularza.';
                    result.className = 'bcs-form-review-result-058 is-error';
                    confirmButton.disabled = false;
                    confirmButton.textContent = 'Spróbuj ponownie';
                    popup(result.textContent, false);
                } finally {
                    verifying = false;
                }
            };
            ...
            const moveCardVerification = () => {
                ...
                const verification = document.querySelector('.bcs-crm-layout section.bcs-form-verification');
                if (!verification) return;
                const panel = Array.from(document.querySelectorAll('.bcs-crm-layout .bcs-accordion-panel')).find((item) => {
                    const heading = item.querySelector('summary strong')?.textContent || '';
                    return /Dane z formularza (?:zgłoszeniowego\s*[–-]\s*)?obozowego/i.test(heading);
                });
                const target = panel?.querySelector('.bcs-accordion-content');
                if (!target) return;
                const form = verification.querySelector('form');
                if (form && !form.querySelector('input[name="bcs_crm_action"]')) {
                    const action = document.createElement('input');
                    action.type = 'hidden';
                    action.name = 'bcs_crm_action';
                    action.value = 'verify_form';
                ...
                verification.classList.add('bcs-form-verification-inline-058');
            ...
            const enhanceListButtons = (root = document) => {
                ...
                root.querySelectorAll?.('form.bcs-list-action').forEach((form) => {
                    ...
                    button.type = 'button';
                    button.dataset.bcsFormReview058 = '1';
                    button.classList.add('bcs-form-review-open-058');
                    button.textContent = 'Potwierdź formularz';
                });
            };

            moveCardVerification();
            enhanceListButtons();
