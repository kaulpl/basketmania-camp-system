...
class BCS_Agreements {
    ...
    public static function build_for_registration(int $registration_id, string $status='pending', bool $include_date=true): int {
        ...
        $replace=[
            ...
            '{{PARENT_PHONE_ALT}}'=>esc_html((string)($reg->second_parent_phone??$reg->parent_phone_alt??'')),'{{PARENT_PHONE_2}}'=>esc_html((string)($reg->second_parent_phone??$reg->parent_phone_alt??'')),'{{PARENTS_NAMES}}'=>esc_html((string)($reg->parents_names??'')),'{{CHILD_ADDRESS}}'=>nl2br(esc_html((string)($reg->child_address??''))),
            '{{SECOND_PARENT_NAME}}'=>esc_html(trim((string)($reg->second_parent_first_name??'').' '.(string)($reg->second_parent_last_name??''))),'{{SECOND_PARENT_EMAIL}}'=>esc_html((string)($reg->second_parent_email??'')),'{{SECOND_PARENT_PHONE}}'=>esc_html((string)($reg->second_parent_phone??'')),'{{SOLE_GUARDIAN}}'=>!empty($reg->sole_guardian)?'tak':'nie',
