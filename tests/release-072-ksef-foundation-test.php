...
if ($currentVersion === '' || ($constantVersion[1] ?? '') !== $currentVersion || version_compare($currentVersion, '0.72', '<')) $fail('Plugin version declarations must be synchronized at 0.72 or newer.');
foreach (['class-bcs-ksef-config.php','class-bcs-ksef-secret.php','class-bcs-ksef-install.php','class-bcs-ksef-client.php','class-bcs-ksef-fa3.php','class-bcs-ksef-admin.php','class-bcs-release-072.php','BCS_Release_072::init();'] as $needle) if (!str_contains($bootstrap, $needle)) $fail('KSeF 0.72 bootstrap is incomplete: '.$needle);

if (BCS_KSeF_Config::TEST_BASE_URL !== 'https://api-test.ksef.mf.gov.pl/v2') $fail('Incorrect KSeF TEST API base URL.');
if (BCS_KSeF_Config::PRODUCTION_BASE_URL !== 'https://api.ksef.mf.gov.pl/v2') $fail('Incorrect KSeF production API base URL.');
...
if (BCS_KSeF_Config::allowed_environment('test') !== 'test') $fail('TEST environment must remain available.');
if (version_compare($currentVersion, '0.76', '>=')) {
    if (BCS_KSeF_Config::allowed_environment('production') !== 'production') $fail('Production environment must be available from 0.76.');
    if (BCS_KSeF_Config::allowed_environment('demo') !== 'test') $fail('DEMO must remain disabled in 0.76.');
} else {
    if (BCS_KSeF_Config::allowed_environment('production') !== 'test') $fail('Pre-0.76 releases must force TEST.');
}

if (!function_exists('sodium_crypto_secretbox')) $fail('Sodium is required for the KSeF secret test.');
$secret = 'ksef-token-that-must-never-be-plaintext';$encrypted = BCS_KSeF_Secret::encrypt($secret);if ($encrypted['ciphertext'] === '' || $encrypted['nonce'] === '' || str_contains($encrypted['ciphertext'], $secret)) $fail('KSeF token was not encrypted safely.');if (BCS_KSeF_Secret::decrypt($encrypted['ciphertext'], $encrypted['nonce']) !== $secret) $fail('KSeF token encryption round-trip failed.');
...
$validation = BCS_KSeF_FA3::validate($sampleXml);if (!$validation['success']) $fail('Structural FA(3) validation failed: '.implode(' ', $validation['errors']));
