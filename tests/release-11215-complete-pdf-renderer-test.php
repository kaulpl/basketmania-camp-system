...
$check(preg_match('/Version: ([0-9.]+)/',$plugin,$header)===1&&preg_match("/BCS_VERSION', '([0-9.]+)'/",$plugin,$constant)===1&&$header[1]===$constant[1]&&version_compare($header[1],'1.12.15','>='),'Wersja 1.12.15 lub nowsza powinna być spójna.');
...
$check(str_contains($pdf,'} elseif (!$preserve_layout) {')&&str_contains($pdf,"if (!\$useAgreementV2) {\n                if (!\$preserve_layout) {"),'Tryb kompletnego PDF nie może uruchamiać historycznych dekoratorów ani renderera umowy.');
