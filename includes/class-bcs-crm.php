...
class BCS_CRM {
    ...
    private static function detail(int $id): void {
        ...
        echo self::registration_data_panel($r);
    ...
    private static function registration_data_panel(object $r): string {
        $rows = [
            ['Opiekun', trim($r->parent_first_name.' '.$r->parent_last_name)],
            ['E-mail kontaktowy', $r->parent_email],
            ['Telefon', $r->parent_phone],
            ['Wybrany turnus', $r->camp_name],
            ['Termin turnusu', $r->start_date.' – '.$r->end_date],
            ['Miejsce', $r->location],
            ...
            ['Data zgłoszenia', !empty($r->created_at) ? BCS_Utils::format_datetime($r->created_at) : ''],
            ['Numer zgłoszenia', '#'.(int)$r->id],
        ];
        $html = '<section class="bcs-panel bcs-accordion-panel bcs-registration-data"><details><summary><span><span class="dashicons dashicons-id"></span><strong>Dane rejestracyjne</strong></span><span class="bcs-accordion-hint">Rozwiń dane</span></summary><div class="bcs-accordion-content"><p class="bcs-muted">Dane podane w pierwszym formularzu rejestracyjnym.</p><div class="bcs-detail-grid bcs-form-preview-grid">';
        foreach ($rows as [$label, $value]) {
            $display = trim((string)$value) !== '' ? (string)$value : '—';
            $html .= '<div class="bcs-detail-item"><span>'.esc_html($label).'</span><strong>'.esc_html($display).'</strong></div>';
        }
    ...
    private static function list_action_form(int $id,string $action,string $label,string $class):string{ob_start();echo '<form method="post" class="bcs-list-action">';wp_nonce_field('bcs_crm_'.$id);echo '<input type="hidden" name="registration_id" value="'.$id.'"><button class="'.esc_attr($class).'" name="bcs_crm_action" value="'.esc_attr($action).'">'.esc_html($label).'</button></form>';return (string)ob_get_clean();}
