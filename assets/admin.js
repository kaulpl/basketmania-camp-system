...
(function(){function esc(v){return String(v||'—').replace(/[&<>"']/g,function(x){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[x]})}function deviceLabel(v){return {mobile:'Urządzenie mobilne',tablet:'Tablet',desktop:'Komputer stacjonarny lub laptop',unknown:'Nie rozpoznano'}[v]||'Nie rozpoznano'}document.addEventListener('click',function(e){var b=e.target.closest('.bcs-registration-preview');if(b){var m=document.getElementById('bcs-registration-preview-modal'),c=m&&m.querySelector('[data-bcs-registration-preview-content]');if(!m||!c)return;var rows=[['Imię Nazwisko',b.dataset.parent],['E-mail',b.dataset.email],['Telefon',b.dataset.phone],['Uczestnik',b.dataset.child],['Data urodzenia',b.dataset.birth],['Wzrost',b.dataset.height?b.dataset.height+' cm':'—'],['Rozmiar stroju',b.dataset.shirt||'—'],['Turnus',b.dataset.camp],['Data zgłoszenia',b.dataset.created],['Urządzenie',deviceLabel(b.dataset.device)]];c.innerHTML=rows.map(function(r){return '<div class="bcs-preview-row"><b>'+esc(r[0])+'</b>: <span>'+esc(r[1])+'</span></div>'}).join('');m.hidden=false;}if(e.target.closest('.bcs-registration-preview-close')||e.target.id==='bcs-registration-preview-modal'){var m=document.getElementById('bcs-registration-preview-modal');if(m)m.hidden=true;}});document.addEventListener('keydown',function(e){if(e.key==='Escape'){var m=document.getElementById('bcs-registration-preview-modal');if(m)m.hidden=true;}});})();
