...
class BCS_Admin {
    ...
    private static function save_registration(): void {
        ...
        $data = [
            ...
            'parent_phone_alt' => sanitize_text_field(wp_unslash($_POST['parent_phone_alt'] ?? ($existing->parent_phone_alt ?? ''))),
            'parents_names' => sanitize_textarea_field(wp_unslash($_POST['parents_names'] ?? ($existing->parents_names ?? ''))),
            ...
            'child_weight' => max(0, (float)($_POST['child_weight'] ?? ($existing->child_weight ?? 0))),
            'child_pesel' => sanitize_text_field(wp_unslash($_POST['child_pesel'] ?? '')),
            'child_address' => sanitize_textarea_field(wp_unslash($_POST['child_address'] ?? ($existing->child_address ?? ''))),
            ...
            'special_educational_needs' => sanitize_textarea_field(wp_unslash($_POST['special_educational_needs'] ?? ($existing->special_educational_needs ?? ''))),
            'vaccination_tetanus' => sanitize_text_field(wp_unslash($_POST['vaccination_tetanus'] ?? ($existing->vaccination_tetanus ?? ''))),
            'vaccination_diphtheria' => sanitize_text_field(wp_unslash($_POST['vaccination_diphtheria'] ?? ($existing->vaccination_diphtheria ?? ''))),
            'vaccination_other' => sanitize_textarea_field(wp_unslash($_POST['vaccination_other'] ?? ($existing->vaccination_other ?? ''))),
            'invoice_requested' => array_key_exists('invoice_requested', $_POST) ? 1 : (int)($existing->invoice_requested ?? 0),
            'invoice_buyer_name' => sanitize_text_field(wp_unslash($_POST['invoice_buyer_name'] ?? ($existing->invoice_buyer_name ?? ''))),
            'invoice_street' => sanitize_text_field(wp_unslash($_POST['invoice_street'] ?? ($existing->invoice_street ?? ''))),
            'invoice_postal_code' => sanitize_text_field(wp_unslash($_POST['invoice_postal_code'] ?? ($existing->invoice_postal_code ?? ''))),
            'invoice_city' => sanitize_text_field(wp_unslash($_POST['invoice_city'] ?? ($existing->invoice_city ?? ''))),
            'invoice_nip' => sanitize_text_field(wp_unslash($_POST['invoice_nip'] ?? ($existing->invoice_nip ?? ''))),
            'invoice_notes' => sanitize_textarea_field(wp_unslash($_POST['invoice_notes'] ?? ($existing->invoice_notes ?? ''))),
    ...
    private static function registration_editor(int $id): void {
        ...
        $fields=[['parent_first_name','Imię opiekuna','text'],['parent_last_name','Nazwisko opiekuna','text'],['parent_email','E-mail','email'],['parent_phone','Telefon I','text'],['parent_phone_alt','Telefon II','text'],['parents_names','Imiona i nazwiska rodziców','text'],['child_first_name','Imię dziecka','text'],['child_last_name','Nazwisko dziecka','text'],['child_birth_date','Data urodzenia','date'],['child_height','Wzrost (cm)','number'],['child_weight','Waga (kg)','number'],['child_pesel','PESEL','text'],['child_club','Klub','text'],['shirt_size','Rozmiar koszulki','text']];foreach($fields as $f)echo '<label><span>'.esc_html($f[1]).'</span><input type="'.esc_attr($f[2]).'" name="'.esc_attr($f[0]).'" value="'.esc_attr((string)($r->{$f[0]}??'')).'" '.(in_array($f[0],['parent_first_name','parent_last_name','parent_email','parent_phone','child_first_name','child_last_name'],true)?'required':'').'></label>';
