 ...
 * a po poprawnym przyjęciu przez KSeF przekazuje PDF rodzicowi niezależnie od środowiska.
 */
final class BCS_KSeF_Invoice_Flow {
    ...
    public static function generate_and_submit(int $registrationId): bool {
        ...
        return self::set_result(true,
            'Faktura została wygenerowana i przesłana do KSeF '.BCS_KSeF_Config::label($environment).'. KSeF nadal ją przetwarza; po przyjęciu PDF zostanie automatycznie wysłany rodzicowi.',
            'processing',
    ...
    public static function finalize(int $invoiceId): void {
        ...
        if ((string)$invoice->ksef_status === 'accepted') {
            self::after_acceptance(
                $invoiceId,
                (int)$invoice->registration_id,
                BCS_KSeF_Config::allowed_environment((string)($invoice->ksef_environment_used ?: 'test')),
                'Faktura została przyjęta przez KSeF.'
            );
            return;
        }
        ...
        if ((string)($result['status'] ?? '') === 'accepted') {
            self::after_acceptance(
                $invoiceId,
                (int)$invoice->registration_id,
                BCS_KSeF_Config::allowed_environment((string)($result['environment'] ?? $invoice->ksef_environment_used ?? 'test')),
    ...
    private static function after_acceptance(int $invoiceId, int $registrationId, string $environment, string $message): bool {
        ...
        // 0.77: po akceptacji przez KSeF PDF wysyłamy rodzicowi zarówno dla TEST, jak i PRODUKCJI.
        // Znacznik ksef_delivery_completed_at chroni przed ponowną wysyłką przy kolejnych odświeżeniach statusu.
        if (empty($invoice->ksef_delivery_completed_at)) {
            $delivery = self::deliver($invoice, $registrationId, $environment);
            if (!$delivery['success']) {
                return self::set_result(
                    false,
                    'KSeF przyjął fakturę, ale nie udało się przekazać PDF rodzicowi: '.$delivery['message'],
                    'accepted',
                    ['invoice_id'=>$invoiceId, 'environment'=>$environment, 'ksef_number'=>(string)$invoice->ksef_number]
                );
            }
        ...
        return self::set_result(
            true,
            $message.' PDF został przekazany rodzicowi.',
            'accepted',
            ['invoice_id'=>$invoiceId, 'environment'=>$environment, 'ksef_number'=>(string)$invoice->ksef_number]
        );
    }

    private static function deliver(object $invoice, int $registrationId, string $environment): array {
        global $wpdb;
